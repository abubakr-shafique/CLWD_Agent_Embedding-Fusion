...
├── run_all_embedding_models.py            <- Automatically run all the models to generate embeddins (for windows).
├── run_all_MIL_models.py                  <- Automatically run MIL on all the embeddings generated (for windows).
├── run_all_Metadata.py                    <- Automatically run all the metadata representation learning, classifier and evaluation (for windows)
├── run_all_slide_embedding_models.py      <- Automatically aggregate and generate the slide level embedding using best MIL model (for windows).
├── run_all_bestMIL_with_Metadata.py       <- Automatically run Best MIL model with metadata (Age + Sex) combined (for windows).
├── run_all_agents.py                      <- Automatically run the fusion agent for all three folds (for windows).
│
...
Furthermore, an AI-based fusion agent was employed to explore multiple feature-fusion architectures and identify an optimal fusion configuration. The agent evaluated different fusion configurations using the validation set, and the configuration achieving the highest validation balanced accuracy was selected for subsequent evaluation on the held-out test set. In this fusion strategy, the ABMIL slide-level embeddings from all three FMs (UNI2-h, Virchow2, and H-OPTIMUS-1) were combined with clinical metadata (sex and age) embedding in an attempt to improve classification performance. Across the three cross-validation folds, the resulting test balanced accuracies were 62.33%, 59.26%, and 68.70%, respectively. Compared with the corresponding manual best ABMIL + metadata classification results, an improvement in balanced accuracy was observed in Folds 0 and 2, whereas Fold 1 showed a slight decrease in performance. <br>

Although the agentic embedding-fusion strategy resulted in improvements in balanced accuracy, the AUROC remained within a similar range, with a mean AUROC of approximately 88%. The manual and agentic embedding-fusion strategies also differ in their underlying feature composition. The manual fusion approach incorporates the slide-level embeddings from the best-performing slide-level aggregator, whereas the agentic fusion strategy integrates slide-level embeddings from all three FMs evaluated in this study (UNI2-h, Virchow2, and H-OPTIMUS-1). Furthermore, the agentic fusion strategy identified different optimal fusion architectures across the individual cross-validation folds, indicating that the most effective combination of feature representations may vary depending on the specific data partition. <br>

Overall, these findings suggest that incorporating clinical metadata through manual fusion, as well as using an agentic fusion strategy to identify an optimal combination of embeddings, can improve balanced accuracy compared with individual slide-level models. However, the magnitude and consistency of these improvements vary across cross-validation folds, while the AUROC remains within a similar range. These results highlight the potential value of multimodal fusion while also indicating that the benefits of combining multiple clinical and morphological representations should be evaluated systematically. Further investigation is warranted to quantify the incremental benefit of incorporating different clinical variables and FM representations relative to single-modality approaches. Importantly, the computational and implementation costs associated with increasingly complex fusion strategies should also be considered. This would enable a more comprehensive assessment of the trade-off between the observed improvement in classification performance and the additional computational resources, model complexity, and inference requirements required to achieve these improvements.

## Failure Analysis
**MetaData Classification:** <br>
Analysis of the clinical metadata and confusion matrix results suggests that the demographic variables, age and sex, did not demonstrate a clear association with the Acinar, Cribriform, or Lepidic subtypes. In contrast, the observed relationships between these clinical variables and model predictions appeared to be more pronounced for patients with In Situ and Solid subtypes. These findings suggest that the contribution of age and sex to subtype discrimination may be heterogeneous across the different histopathological subtypes. <br>

**ABMIL Classification:** <br>
The ABMIL model using H-OPTIMUS-1 embeddings demonstrated variation in the learned associations between tissue morphology and tumour subtypes across the different training data splits. In Fold 0, the slide-level aggregation exhibited strong associations with the In Situ, Micropapillary, Papillary, and Solid subtypes, while also capturing weaker associations with the remaining classes. Notably, Acinar cases were frequently misclassified as Papillary, whereas Micropapillary cases showed confusion with the Cribriform subtype. <br>

In Fold 1, strong associations were observed for the In Situ, Solid, and Papillary subtypes. Similar to Fold 0, Acinar cases were misclassified primarily as Papillary and Lepidic, while Micropapillary cases were frequently confused with Cribriform. In Fold 2, the model generally demonstrated strong associations between the learned representations and their corresponding tumour subtypes. However, some inter-class confusion remained, with Papillary cases being misclassified as Micropapillary and Acinar cases being misclassified as Solid, Papillary, and Cribriform. <br>

Overall, these observations indicate that the learned morphological associations are influenced by the training data partition, with certain subtype-specific patterns being consistently learned across folds while others exhibit greater variability. In particular, the recurrent confusion involving Acinar, Papillary, Micropapillary, and Cribriform subtypes may reflect similarities in their learned morphological representations or overlapping histopathological features.

**ABMIL + Metadata Classification:** <br>
The incorporation of clinical metadata with the ABMIL H-OPTIMUS-1 slide-level representations resulted in changes to the strength and distribution of the learned subtype associations across the cross-validation folds. In Fold 0, the signals associated with the Acinar, In Situ, and Micropapillary subtypes became more pronounced. However, this was accompanied by additional inter-class confusion, with Acinar cases being misclassified as Lepidic and Cribriform cases showing confusion with Solid and other subtypes. In Fold 1, stronger associations were observed for the In Situ, Lepidic, Micropapillary, and Solid subtypes. Nevertheless, Cribriform cases were frequently misclassified as In Situ and Solid, while Micropapillary cases were confused with Cribriform and Papillary cases showed misclassification across Acinar, Micropapillary, and Lepidic subtypes. <br>

In Fold 2, stronger signals were observed for the Acinar, Cribriform, and In Situ subtypes. However, some inter-class confusion remained, with Acinar cases being misclassified as Cribriform and Micropapillary, while Solid cases were misclassified as Cribriform and Micropapillary. Additionally, Lepidic cases showed confusion with the In Situ subtype. <br>

Overall, the effect of incorporating clinical metadata was not consistent across the cross-validation folds and appeared to depend on the underlying data stratification. Although the multimodal representation strengthened the learned associations for certain subtypes, particularly In Situ, these improvements were accompanied by changes in the misclassification patterns between other subtypes. This suggests that the addition of clinical metadata may modify the decision boundaries learned from the morphological representations rather than producing a uniform improvement across all tumour subtypes. <br>

## Future Work (with 10× the compute)
With access to substantially greater computational resources, future work could investigate domain-specific fine-tuning of self-supervised FMs using the available lung adenocarcinoma dataset. Rather than relying exclusively on frozen, pretrained representations, self-supervised adaptation on target-domain WSIs could enable the model to learn morphological characteristics that are particularly relevant to lung adenocarcinoma and its seven histological subtypes. This may improve the domain specificity of the learned representations while reducing reliance on representations learned from heterogeneous external datasets.<br>
